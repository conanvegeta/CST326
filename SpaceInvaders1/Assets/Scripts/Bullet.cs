...
public class Bullet : MonoBehaviour
{
	...
	void OnTriggerEnter2D(Collider2D col)
	{
		...
		if (col.gameObject.tag == "Alien4")
		{
			SoundManager.Instance.PlayOneShot(SoundManager.Instance.alienDies);

			// Increase the Score Text component
			increaseTextUIScore4();
	...
	void increaseTextUIScore()
	{
		...
		if(score<100)
		textUIComp.text = "00"+score.ToString();
		else
			textUIComp.text = "0" + score.ToString();

		if (score > highScore)
			highScore = score;
		textUIComp = GameObject.Find("HiScore").GetComponent<Text>();

		if (highScore < 100)
			textUIComp.text = "00" + highScore.ToString();
		else
			textUIComp.text = "0" + highScore.ToString();
	}
